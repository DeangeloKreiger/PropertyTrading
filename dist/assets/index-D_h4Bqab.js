import{a as N,s as L,i as x,d as $,e as f,f as T,h as v,r as g,j as _,k as S,l as B,m as F,n as D}from"./wagmi-D7Jml_7m.js";import{h as A}from"./viem-CNhNOX-L.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))r(s);new MutationObserver(s=>{for(const o of s)if(o.type==="childList")for(const c of o.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&r(c)}).observe(document,{childList:!0,subtree:!0});function n(s){const o={};return s.integrity&&(o.integrity=s.integrity),s.referrerPolicy&&(o.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?o.credentials="include":s.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function r(s){if(s.ep)return;s.ep=!0;const o=n(s);fetch(s.href,o)}})();const O=()=>{const e=localStorage.getItem("walletconnect_project_id");return e||null},k=O(),i=N({chains:[L],connectors:k?[x(),$({projectId:k})]:[x()],transports:{[L.id]:A()}}),U=e=>{localStorage.setItem("walletconnect_project_id",e),window.location.reload()},R=()=>localStorage.getItem("walletconnect_project_id")||"";function P(e,t="info",n=3e3){const r=document.querySelector(".toast");r&&r.remove();const s=document.createElement("div");s.className=`toast toast-${t}`,s.textContent=e,document.body.appendChild(s),setTimeout(()=>{s.remove()},n)}function u(e){P(e,"error",4e3)}function y(e){P(e,"success",3e3)}function E(e){P(e,"info",3e3)}var W=Object.defineProperty,z=(e,t,n)=>t in e?W(e,t,{enumerable:!0,configurable:!0,writable:!0,value:n}):e[t]=n,M=(e,t,n)=>z(e,typeof t!="symbol"?t+"":t,n);class q{constructor(){M(this,"element",null),M(this,"overlay",null)}show(t="Loading..."){this.hide(),this.overlay=document.createElement("div"),this.overlay.className="modal-overlay",this.element=document.createElement("div"),this.element.className="modal-content flex flex-col items-center gap-4",this.element.innerHTML=`
  ...
  `}function re(){document.getElementById("connect-btn")?.addEventListener("click",se),document.getElementById("disconnect-btn")?.addEventListener("click",oe),document.getElementById("register-property-btn")?.addEventListener("click",ie),document.getElementById("settings-btn")?.addEventListener("click",ue),document.getElementById("tab-marketplace")?.addEventListener("click",()=>w("marketplace")),document.getElementById("tab-my-properties")?.addEventListener("click",()=>w("my-properties")),document.getElementById("tab-history")?.addEventListener("click",()=>w("history")),S(i,{onChange(e){l=e.address,b()}})}async function se(){try{a.show("Connecting to wallet..."),await F(i,{connector:x()}),l=B(i).address,b(),y("Wallet connected successfully!"),a.hide()}catch(e){a.hide(),u(e.message||"Failed to connect wallet")}}async function oe(){try{await D(i),l=void 0,b(),E("Wallet disconnected")}catch(e){u(e.message||"Failed to disconnect")}}function ae(){const e=B(i);e.isConnected&&(l=e.address,b())}function b(){const e=document.getElementById("connect-btn"),t=document.getElementById("account-info"),n=document.getElementById("account-address"),r=document.getElementById("account-initial");if(l){if(e?.classList.add("hidden"),t?.classList.remove("hidden"),n){const s=`${l.slice(0,6)}...${l.slice(-4)}`;n.textContent=s}r&&(r.textContent=l.slice(2,3).toUpperCase()),m(),h()}else e?.classList.remove("hidden"),t?.classList.add("hidden")}async function m(){const e=document.getElementById("listed-properties");if(e)try{a.show("Loading properties...");const t=await K();if(console.log("Total properties:",t),t===BigInt(0)){e.innerHTML='<p class="text-center col-span-full" style="color: var(--color-text-secondary)">No properties registered yet. Register your first property!</p>',a.hide();return}const n=await V();if(n.length===0){e.innerHTML='<p class="text-center col-span-full" style="color: var(--color-text-secondary)">No properties listed for sale yet</p>',a.hide();return}const r=[];for(const s of n)try{const o=await C(Number(s));r.push(o)}catch(o){console.error(`Failed to load property ${s}:`,o)}e.innerHTML=r.map(s=>j(s,!1)).join(""),r.forEach(s=>{document.getElementById(`purchase-${s.id}`)?.addEventListener("click",()=>pe(s))}),a.hide()}catch(t){a.hide(),console.error("Error loading properties:",t);const n=t?.message||"Unknown error";n.includes("reverted")||n.includes("execution")?e.innerHTML=`
  ...
  `,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.remove()}),document.getElementById("close-modal")?.addEventListener("click",()=>e.remove()),document.getElementById("cancel-register")?.addEventListener("click",()=>e.remove()),document.getElementById("register-form")?.addEventListener("submit",async t=>{t.preventDefault();const n=document.getElementById("property-hash").value,r=document.getElementById("property-price").value;e.remove();try{a.show("Registering property on blockchain..."),await Q(n,r),a.hide(),y(`Property registered successfully! Hash: ${n}, Price: ${r} ETH`),await m(),await h()}catch(s){a.hide();const o=s?.message||"Failed to register property";u(o),console.error("Registration error:",s)}})}async function ce(e){try{a.show("Listing property for sale..."),await X(Number(e)),a.hide(),y("Property listed successfully!"),await m(),await h()}catch(t){a.hide(),u(t?.message||"Failed to list property")}}async function le(e){try{a.show("Delisting property..."),await J(Number(e)),a.hide(),y("Property delisted successfully!"),await m(),await h()}catch(t){a.hide(),u(t?.message||"Failed to delist property")}}async function de(e){const t=prompt("Enter new price in ETH:",I(e.price));if(t)try{a.show("Updating price..."),await Z(Number(e.id),t),a.hide(),y(`Price updated to ${t} ETH!`),await m(),await h()}catch(n){a.hide(),u(n?.message||"Failed to update price")}}async function pe(e){const t=I(e.price);if(confirm(`Purchase this property for ${t} ETH?`))try{a.show("Purchasing property..."),await ee(Number(e.id),t),a.hide(),y("Property purchased successfully!"),await m(),await h()}catch(r){a.hide(),u(r?.message||"Failed to purchase property")}}function ue(){const e=document.createElement("div");e.className="modal-overlay";const t=R();e.innerHTML=`
  ...
  `,document.body.appendChild(e),e.addEventListener("click",n=>{n.target===e&&e.remove()}),document.getElementById("close-settings-modal")?.addEventListener("click",()=>e.remove()),document.getElementById("cancel-settings")?.addEventListener("click",()=>e.remove()),document.getElementById("settings-form")?.addEventListener("submit",n=>{n.preventDefault();const r=document.getElementById("project-id-input").value.trim();r?(U(r),y("Settings saved! Page will reload to apply changes.")):(E("Project ID cleared. Only MetaMask will be available."),localStorage.removeItem("walletconnect_project_id"),window.location.reload())})}document.addEventListener("DOMContentLoaded",te);
